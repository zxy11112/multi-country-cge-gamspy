...
14-region x 12-sector CGE model, benchmark year 2022.

Regions: CHN USA JPN KOR EU27 VNM THA MYS IDN SGP PHL IND MEX ROW
(13 explicit + ROW). Identical methodology to the 6-region version
(prepare_real_data_6x12.py); differences:
  - trade reads BOTH the main cleaned Comtrade long table (5 original
    reporters) and clean_comtrade_sitc_2022_expansion.csv (8 new);
  - EU27 keeps the Comtrade extra-EU control-total override;
  - services shares come from data_real_g20/services_flows_batis_2022.csv
    (prepare_batis_services_14x12.py).
"""
...
COMTRADE_EXP = ROOT / "整理" / "02_清洗" / "clean_comtrade_sitc_2022_g20.csv"
...
def load_trade() -> pd.Series:
    ...
    use = use[(use["period"] == YEAR) & (use["cl_code"] == "S4")
              & (use["cmd_code"] != "TOTAL")]
    # add the expansion reporters (2022 only)
    if COMTRADE_EXP.exists():
        exp = pd.read_csv(COMTRADE_EXP, usecols=cols,
                          dtype={"reporter_iso3": str, "partner_iso3": str,
                                 "flow_code": str, "cmd_code": str})
        exp = exp[(exp["period"] == YEAR) & (exp["cl_code"] == "S4")
                  & (exp["cmd_code"] != "TOTAL")]
        use = pd.concat([use, exp], ignore_index=True)
    use = use[use["reporter_iso3"].isin(R5)]
...
def main() -> None:
    ...
    row_countries = [c for c in all_countries
                     ...
                     + ["IND", "CAN", "MEX", "BRA", "ZAF", "RUS", "SAU", "AUS"]]
    ...
    agg: dict[str, dict] = {}
    for reg in ["CHN", "USA", "JPN", "KOR", "IND", "CAN", "MEX", "BRA",
                "ZAF", "RUS", "SAU", "AUS"]:
    ...
    for k, sec in enumerate(SECTORS):
        ...
        for oi, o in enumerate(R5):
            ...
            if goods:
                if o == "RUS":
                    # Russia did not report to Comtrade in 2022 (sanctions):
                    # build destination shares from partners' import records
                    # (mirror, CIF)
                    mir = {s_: t(s_, "RUS", "M", sec) for s_ in R5 if s_ != o}
                    denom = sum(mir.values())
                    sh = {s_: (v / denom if denom > 0 else 0.0)
                          for s_, v in mir.items()}
                else:
                    denom = t(o, "WLD", "X", sec)
    ...
    for oi, reg in enumerate(REGIONS):
        for k, sec in enumerate(SECTORS):
            ...
            dom = y - x0[oi, k, :].sum()
            if dom < 1.0:
                # zero/tiny-output origin-sector (e.g. SGP MIN): keep the
                # cell alive at epsilon scale; output is lifted accordingly
                dom = 1.0
                y = float(x0[oi, k, :].sum()) + dom
                agg[reg]["output"][sec] = y
                print(f"  note: {reg}/{sec} lifted to epsilon output "
                      f"({y:,.0f})")
        ...
        for i_out in SECTORS:
            ...
            for j_in in SECTORS:
                io_rows.append((j_in, reg, i_out,
                                float(a["ttl_int"].loc[j_in, i_out])
                                / y if y > 0 else 0.0))
